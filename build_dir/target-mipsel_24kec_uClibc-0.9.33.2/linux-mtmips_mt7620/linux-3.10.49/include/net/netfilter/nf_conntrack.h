...
struct nf_conn {
	...
	/* Used for HiWiFi modules to store private information. */
	char hwf_ext[8] __aligned(8);
#endif
	...
	#if defined(CONFIG_NETFILTER_XT_TARGET_PROSTAT)||\
		defined(CONFIG_NETFILTER_XT_TARGET_PROSTAT_MODULE)
		uint32_t user_id;
	#endif
	
	#if defined(CONFIG_NETFILTER_HIWIFI_DPI)||\
		defined(CONFIG_NETFILTER_HIWIFI_DPI_MODULE)
	unsigned int flag_status;
	unsigned int fw_status;
	u_int32_t fw_parent_appid;
	u_int32_t fw_appid;
	u_int64_t p2p_packets[IP_CT_DIR_MAX], p2p_bytes[IP_CT_DIR_MAX];
	u8 before_state;
	u8 current_state;
	u8  app_flag;
	#endif

	unsigned char user_key[ETH_ALEN];
