...
#include <linux/kjson/json_inttypes.h>
...
/**
 * Decrement the reference count of json_object and free asynchronously if it reaches zero.
 * You must have ownership of obj prior to doing this or you will cause an
 * imbalance in the reference count.
 *
 * @param obj the json_object instance
 * @returns 1 if the object was freed.
 */
int json_object_release(struct json_object *obj);
...
extern struct json_object* hi_json_object_new_string_reverse(const char *s);

extern unsigned int hi_json_object_get_uint(struct json_object *jso);
