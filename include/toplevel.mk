...
prepare-tmpinfo: FORCE
	mkdir -p tmp/info
	$(_SINGLE)$(NO_TRACE_MAKE) -j1 -r -s -f include/scan.mk SCAN_TARGET="packageinfo" SCAN_DIR="package" SCAN_NAME="package" SCAN_DEPS="$(TOPDIR)/include/package*.mk $(TOPDIR)/overlay/*/*.mk" SCAN_DEPTH=8 SCAN_EXTRA=""
...
# HiWiFi config
%_hiwifi: scripts/config/conf prepare-tmpinfo FORCE
	if [ \! -e $(TOPDIR)/hcconfig/$(@:_hiwifi=) ] ; then \
		$(_SINGLE)$(NO_TRACE_MAKE) menuconfig $(PREP_MK); \
	fi
	$(TOPDIR)/scripts/hcconfig.sh $(TOPDIR)/hcconfig $(TOPDIR)/hcconfig/$(@:_hiwifi=) $(TOPDIR)/.config
