...
enable_mtmips() {
	...
	find_mtmips_phy "$device" || return 0
...
post_mtmips() {
	...
	# if led is disable; just disable wlan led
 	if [ $? -eq 0 ]; then
		iwpriv ra0 set WlanLed=0 >/dev/null 2>&1
...
detect_mtmips() {
	...
	[ -n "$type" ] || detect_mtmips_ra "radio0"
	config_get type "radio1" type

	[ -n "$type" ] || {
		grep -qs "^ *rai0:" /proc/net/dev && detect_mtmips_rai "radio1"
	}
}
